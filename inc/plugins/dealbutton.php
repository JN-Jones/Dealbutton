...
//    echo "<pre>";var_dump($post);echo "</pre>";
    ...
    if(in_array($post['fid'], $fids)||(sizeOf($fids)==1&&$fids[0]==0)) {
        if($post['replyto']!=0) {
            $query = $db->simple_select("posts", "uid", "pid='{$post['replyto']}'");
...
function dealbutton_create()
{
    ...
    if($mybb->input['action']=="dealbutton") {
        ...
        $name = $db->fetch_field($db->simple_select("users", "username", "uid='{$mybb->input['uid']}'"), "username");
        $aname = $db->fetch_field($db->simple_select("users", "username", "uid='{$mybb->input['auid']}'"), "username");
        $osubject = $db->fetch_field($db->simple_select("posts", "subject", "pid='{$mybb->input['pid']}'"), "subject");
